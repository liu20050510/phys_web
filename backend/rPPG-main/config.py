...
# -----------------------------------------------------------------------------\
_C.TEST = CN()
_C.TEST.OUTPUT_SAVE_DIR = './videoPredictions'#
_C.TEST.METRICS = []
...
_C.TEST.DATA.PREPROCESS.CHUNK_LENGTH = 90
...
def update_config(config, args):
    ...
    # 新增数据路径注入逻辑
    if hasattr(args, 'data_path') and args.data_path:  # 检查是否传入了data_path参数
        config.defrost()
        # 根据TOOLBOX_MODE选择注入路径的位置
        if config.TOOLBOX_MODE == "inference":
            config.TEST.DATA.DATA_PATH = args.data_path
        elif config.TOOLBOX_MODE == "unsupervised_method":
            config.UNSUPERVISED.DATA.DATA_PATH = args.data_path
        else:
            config.TRAIN.DATA.DATA_PATH = args.data_path
        config.freeze()
