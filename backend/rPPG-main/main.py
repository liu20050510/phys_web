...
def add_args(parser):
    ...
    parser.add_argument('--config_file', required=True, type=str, help="配置文件路径")
    parser.add_argument('--data_path', type=str, help="动态数据路径")  # 新增参数
    return parser
...
def inference(config, data_loader_dict):
    if config.MODEL.NAME == 'RhythmMamba':
        model_trainer = trainer.RhythmMambaTrainer.RhythmMambaTrainer(config, data_loader_dict)
    else:
        raise ValueError('Your Model is Not Supported  Yet!')
    model_trainer.inference(data_loader_dict)
...
if __name__ == "__main__":
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        ...
        if config.TEST.DATA.DATASET == "SingleVideo":
            inference_loader = data_loader.userLoader.SingleVideoLoader
        if config.TEST.DATA.DATASET and config.TEST.DATA.DATA_PATH:
            inference_data = inference_loader(
                name="inference",
                data_path=config.TEST.DATA.DATA_PATH,
                config_data=config.TEST.DATA)
            data_loader_dict["inference"] = DataLoader(
            ...
            data_loader_dict['inference'] = None
        ...
        elif config.TEST.DATA.DATASET == "SingleVideo":
            test_loader = data_loader.userLoader.SingleVideoLoader
        else:
            raise ValueError("Unsupported dataset! Currently supporting UBFC-rPPG, PURE, MMPD, \
                             SCAMPS, BP4D+ (Normal and BigSmall preprocessing), and UBFC-PHYS.")
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        ...
        inference(config, data_loader_dict)
    elif config.TOOLBOX_MODE == "unsupervised_method":
